...
class DataFetcher:
    def __init__(self, cache_ttl=3600):
        ...
        self.market_hours = {
            # Major forex pairs trade 24/5
            'major': {'weekday': True, 'weekend': False},
            # Exotic pairs might have limited hours
            'exotic': {'weekday': True, 'weekend': False}
        }
    ...
    def _is_trading_day(self, date, currency_pair):
        """Check if given date is a trading day for the currency pair"""
        dt = pd.to_datetime(date)
        
        # Determine pair type (simple logic - can be expanded)
        pair_type = 'major' if currency_pair in ['usd', 'eur', 'gbp', 'jpy'] else 'exotic'
        
        # Check if weekend
        is_weekend = dt.weekday() >= 5
        
        return self.market_hours[pair_type]['weekend'] if is_weekend else self.market_hours[pair_type]['weekday']
    ...
    def fetch_historical_rates(self, base_currency, days=14):
        ...
        while days_fetched < days and days_checked < days * 2:  # Allow checking up to 2x days to handle non-trading days
            date = (current_date - timedelta(days=days_checked)).strftime('%Y-%m-%d')
            
            if self._is_trading_day(date, base_currency):
                data = self.fetch_rates(base_currency, date)
                if data:
        ...
        logger.info(f"Fetched {len(rates_data)} days of historical data for {base_currency}")
