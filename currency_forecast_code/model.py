...
class CurrencyForecaster:
        ...
        try:
            ...
            self.pipeline = Pipeline([
                ...
                ('model', LGBMRegressor(
                    ...
                    min_child_samples=3, # Minimum number of data needed in a child (leaf).
    ...
    def predict(self, df, forecast_days=7):
        ...
        try:
            ...
            predictions = []
            last_rate = df['rate'].iloc[-1] # Last rate in the historical data
            current_data_for_prediction = df.copy()
            
            # Minimum rows needed for feature creation, + 5 for buffer
            ...
            for i in range(forecast_days):
                ...
                if len(current_data_for_prediction) > min_rows_for_features:
                    data_slice_for_features = current_data_for_prediction.iloc[-min_rows_for_features:] # takes the last min_rows_for_features rows
                else:
                ...
                if next_day_features_df.empty or not all(col in next_day_features_df.columns for col in self.feature_columns):
                    ...
                    next_day_feature_values = next_day_features_df[self.feature_columns].iloc[-1:] # takes the last row
                    pct_change = self.pipeline.predict(next_day_feature_values)[0] # pass the last row to the model to predict the next day's percentage change
                ...
                next_rate = last_rate * (1 + pct_change) # multiply the last rate by the percentage change to get the next day's rate
